...
const API_BASE_URL = 'http://localhost:8000'; // Consider changing for production deployment
...
function showAlert(message, type = 'info', duration = 5000) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => alertDiv.remove(), 400); // Allow time for transition
    }, duration - 400); // Start fading out 400ms before removal
}
...
function showModal(title, message, onConfirm) {
    ...
    modal.element.style.display = 'flex'; // Use display flex to show
    modal.element.setAttribute('aria-hidden', 'false');
    modal.confirmBtn.focus(); // Focus confirm button for accessibility
...
async function performUploadDocuments() {
    ...
    try {
        ...
        showAlert(`${uploadedCount} dokumen berhasil diunggah! ${sanitizeText(data.message || '')}`, 'success', 5000);
...
async function deleteAllChatHistory() {
    ...
    try {
        ...
        showAlert(`✅ Semua riwayat chat berhasil dihapus! (${deletedCount} item dihapus)`, 'success', 5000);
